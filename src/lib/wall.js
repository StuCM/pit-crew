...
export const snapshot = (cfg) => ({
  project: cfg.project,
  models: cfg.models,
  work: readJson(join(cfg.root, WORK_FILE), null),
...
/**
 * The scout, as a `claude -p` invocation. Its own agent file is the prompt,
 * so the page talks to exactly the agent the orchestrator spawns — and it
 * keeps that agent's read-only tools, whatever the caller's settings allow.
 * The message goes in on stdin, where nothing in it can be read as a flag.
 */
export const scoutArgs = (agentFile, sessionId) => {
  ...
  const agents = {
    'crew-scout': { description: field('description') ?? 'Read-only scout', prompt },
  };
  const args = [
    '-p',
    '--output-format',
